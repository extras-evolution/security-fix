...
class AjaxSearchOutput
{
    ...
    public $chkGrpResult;
    public $varGrpResult = array();
    public $tplGrpRes;
    public $chkResult;
    public $varResult = array();
    public $tplRes;
    public $scMain = array();
     ...
     * @return mixed|string
     */
    public function display($validSearch, &$msgErr)
    {
        global $modx;
    ...
    /**
     * Check output field params
     */
    public function _checkParams()
    {
        if (($this->asCfg->cfg['pagingType'] < 0) || ($this->asCfg->cfg['pagingType'] > 2)) {
            $this->asCfg->cfg['pagingType'] = 1;
        }
    }
     ...
     * @return string
     */
    public function _initDisplayResults()
    {
     ...
     * @param $validSearch
     * @param $msgErr
     * @param $nbResultsInfos
     * @return mixed|string
     */
    public function _displayResults($validSearch, &$msgErr, &$nbResultsInfos)
    {
        ...
        if ($this->asCfg->cfg['showResults']) {
            if ($validSearch) {
                ...
                if ($this->asResults->nbResults > 0) {
                    ...
                    for ($ig = 0; $ig < $this->asResults->nbGroups; $ig++) {
                        ...
                        if ($this->asCfg->isAjax) {
                            $nbMax = ($this->asCfg->cfg['ajaxMax'] > 0) ? $this->asCfg->cfg['ajaxMax'] : $nbrs;
                        } else {
                            $nbMax = ($this->asCfg->cfg['grabMax'] > 0) ? $this->asCfg->cfg['grabMax'] : $nbrs;
                        }
                    ...
                    if ($nbFoundResults) {
                        ...
                        if ($this->asCtrl->searchString && ($this->asCtrl->advSearch != NOWORDS)) {
                            ...
                            if ($this->asResults->withExtract) {
                                $searchList = $this->asCtrl->getSearchWords($this->asCtrl->searchString,
                        ...
                        if ($this->asCfg->isAjax) {
                            $this->_setMoreResultsLink($this->asResults->nbResults);
                        }
                    } else {
            ...
            if ($this->needsConvert) {
                $outputResults = mb_convert_encoding($outputResults, "UTF-8", $this->asCfg->pgCharset);
            }
     ...
     */
    public function _displayGrpResult($ig, $site, $subsite, $display, $nbrs, $searchResults, $offset, $nbMax)
    {
        $found = array();
     ...
     * @return mixed
     */
    public function _displayHeaderGrpResult($site, $subsite, $display, $nbrs, $searchResults, $offset, $nbMax)
    {
        $varHeader = array();
     ...
     * @return string
     */
    public function _displayListResults($site, $subsite, $display, $nbrs, $searchResults, & $found, $offset)
    {
     ...
     * @return mixed|null|string|string[]
     */
    public function _displayFooterGrpResult($ig, $nbrs, $offset, $nbMax)
    {
        global $modx;
        $footer = '';
        ...
        if ($nbMax > 0) {
            ...
            if ($pagingType == 0 && !$this->asCfg->isAjax) {
                ...
                if ($this->dbgTpl) {
                    $this->asUtil->dbgRecord($chkPaging->getTemplate($tplPaging), "tplPaging template " . $tplPaging);
                }
                ...
                if ($pagingType == 1 && ($nbrs >= $nbMax || $showPagingAlways)) {
                    ...
                    if ($offset - $nbMax >= 0) {
                        ...
                        if ($this->asCfg->isAjax) {
                            $varPaging1['pagingPrev'] = 'javascript:void(0);';
                        } else {
                    ...
                    if ($offset + $nbMax < $nbrs) {
                        ...
                        if ($this->asCfg->isAjax) {
                            $varPaging1['pagingNext'] = 'javascript:void(0);';
                        } else {
     ...
     * @param $nbrs
     */
    public function _setMoreResultsLink($nbrs)
    {
        global $modx;
     ...
     */
    public function _getCleanCssId($name)
    {
    ...
    public function _getParamsUrl()
    {
        ...
        if ($this->asCfg->cfg['asId']) {
            $url = $firstarg . 'asid=' . urlencode($this->asCfg->cfg['asId']);
        }

        if ($this->asCtrl->searchString) {
            if ($url) {
                $url .= '&search=' . urlencode($this->asCtrl->searchString) . '&amp;advsearch=' . urlencode($this->asCtrl->advSearch);
            } else {
                $url = $firstarg . 'search=' . urlencode($this->asCtrl->searchString) . '&amp;advsearch=' . urlencode($this->asCtrl->advSearch);
            }
        }
        if ($this->asCtrl->subSearch) {
            if (is_array($this->asCtrl->subSearch)) {
                foreach ($this->asCtrl->subSearch as $k => $v) {
                    if ($url) {
                        $url .= '&amp;subsearch=' . urlencode($v);
                    } else {
                        $url = $firstarg . 'subsearch=' . urlencode($v);
                    }
                }
            } else {
                if ($url) {
                    $url .= '&amp;subsearch=' . urlencode($this->asCtrl->subSearch);
                } else {
                    $url = $firstarg . 'subsearch=' . urlencode($this->asCtrl->subSearch);
                }
            }
        }
        if ($this->asCtrl->asf) {
            if ($url) {
                $url .= '&amp;asf=' . urlencode($this->asCtrl->asf);
            } else {
                $url = $firstarg . 'asf=' . urlencode($this->asCtrl->asf);
            }
     ...
     */
    public function _initCommonChunks()
    {
        global $modx;
        ...
        if (!$this->asCfg->isAjax) {
            ...
            if ($tplResults == '') {
                $tplResults = "@FILE:" . AS_SPATH . 'templates/ajaxResults.tpl.html';
            }
        }
    ...
    public function _initChunks()
    {
        if (!$this->asCfg->isAjax) {
            ...
            if ($tplResult == '') {
                $tplResult = "@FILE:" . AS_SPATH . 'templates/ajaxResult.tpl.html';
            }
        }
     ...
     */
    public function _getSearchContext()
    {
     ...
     */
    public function _initDisplayVariables()
    {
        $this->_initChunks();
     ...
     * @return string
     */
    public function _setFailedSearches($asCall = '', $select = '')
    {
        global $modx;
        $logid = '';
        if ($this->log >= 1) {
     ...
     * @return string
     */
    public function _setSuccessfullSearches($ig)
    {
        global $modx;
        $logid = '';
        if ($this->log == 2) {
     ...
     * @return string
     */
    public function _getAsCall($ucfg)
    {
        $call_array = explode(' ', $ucfg);
        $tpl = "&%s=`%s`";
        if ($this->asCtrl->advSearch != ONEWORD) {
            $call_array[] = sprintf($tpl, 'advSearch', $this->asCtrl->advSearch);
        }
        $asCall = "[!AjaxSearch";
        if (count($call_array)) {
            $asCall .= "? ";
    ...
    public function _setResultDisplayed($row)
    {
        ...
        if (isset($this->scMain['date'])) {
            foreach ($this->scMain['date'] as $field) {
                $this->_setPhxField($field, $row, 'date');
            }
        }

        foreach ($this->scMain['displayed'] as $field) {
            $this->_setPhxField($field, $row, 'string');
        }

        if (isset($this->scTvs['names'])) {
            foreach ($this->scTvs['names'] as $field) {
                if (isset($row[$field])) {
                    $this->_setPhxField($field, $row, 'string');
                }
            }
        ...
        if ($this->scMain['append']) {
            foreach ($this->scMain['append'] as $field) {
                if (isset($row[$field])) {
                    $this->_setPhxField($field, $row, 'string');
                }
            }
        ...
        if ($this->asCfg->cfg['rank']) {
            $this->_setPhxField('rank', $row, 'int');
        }
     ...
     * @param string $type
     */
    public function _setPhxField($field, $row, $type = 'string')
    {
     ...
     * @param $row
     */
    public function _setResultLink($row)
    {
        global $modx;
        $id = $this->scMain['id'];
        if (!$row[$id]) {
     ...
     * @param $row
     */
    public function _setResultExtract($row)
    {
     ...
     * @param $row
     */
    public function _setResultBreadcrumbs($row)
    {
        global $modx;
     ...
     * @param $no
     */
    public function _setResultNumber($no)
    {
     ...
     * @return bool
     */
    public function _snippet_exists($snippetName)
    {
        global $modx;
     ...
     * @return string
     */
    public function _getOtherOffset($ig)
    {
        $otherOffset = array();
     ...
     * @param $flag
     */
    public function setNeedsConvert($flag)
    {
        $this->needsConvert = $flag;
    }

    /**
     * initialize the required Class values
     */
    public function initClassVariables()
    {

        if ($this->asCfg->cfg['ajaxSearch']) {
            $this->asClass['prefix'] = PREFIX_AJAX_RESULT_CLASS;
        } else {
            $this->asClass['prefix'] = PREFIX_RESULT_CLASS;
        }
     ...
     * @return string
     */
    public function _getHighlightClass($search, $advSearch)
    {
        $hClass = '';
     ...
     */
    public function setAjaxSearchHeader()
    {
        global $modx;
        ...
        if ($typeAs) {
            if ($this->asCfg->cfg['jscript'] == 'jquery') {
                ...
                if ($this->asCfg->cfg['addJscript']) {
                    $modx->regClientStartupScript($this->asCfg->cfg['jsMooTools2']);
                }
                $jsInclude = AS_SPATH . AJAXSEARCH_JSDIR . $typeAs . '/ajaxSearch-mootools2.js';
            } else {
                if ($this->asCfg->cfg['addJscript']) {
                    $modx->regClientStartupScript($this->asCfg->cfg['jsMooTools']);
                }
...
EOD;
     ...
     * @return mixed|null|string|string[]
     */
    public function _displayComment()
    {
        $outputComment = '';
        if ($this->asLog->logcmt && count($this->logIds)) {
            $chkCmt = new AsPhxParser($this->asCfg->cfg['tplComment']);
            if ($this->dbgTpl) {
                $this->asUtil->dbgRecord($chkCmt->getTemplate($this->asCfg->cfg['tplComment']),
                    "tplComment template " . $this->asCfg->cfg['tplComment']);
            }
            $varCmt = array();
    ...
    /**
     * display the previous / next page of results
     *
     * @param $validSearch
     * @param $msgErr
     * @return string
     */
    public function paginate($validSearch, &$msgErr)
    {
        $ouputResults = null;
        if ($validSearch) {
            ...
            if ($pagingType == 1) {
                ...
                if ($sens > 0) {
                    if ($currentOffset == 0) {
                        $currentNbRes = $ajaxMax;
                    } else {
                        $currentNbRes = NB_MORE_RESULTS;
                    }
                    $offset = $currentOffset + $currentNbRes;
                    $nbRes = ($offset + NB_MORE_RESULTS > $nbrs) ? $nbrs - $offset : NB_MORE_RESULTS;
                } else {
                    if ($currentOffset == $ajaxMax) {
                        $previousNbRes = $ajaxMax;
                    } else {
                        $previousNbRes = NB_MORE_RESULTS;
                    }
            ...
            if ($pagingType == 1) {  // Prev / Next links
                $grpResult = $this->_displayGrpResult($ig, $site, $subsite, $display, $nbrs, $searchResults, $offset,
                    $nbRes);
                ...
                $header = $this->_displayHeaderGrpResult($site, $subsite, $display, $nbrs, $searchResults, $moreOffset,
                    $moreNbMax);
     ...
     * @return string
     */
    public function _getJsonArray($array)
    {
        $nbr = count($array);
        if ($nbr) {
            $jsonArray = '[' . implode(',', $array) . ']';
        } else {
            $jsonArray = '';
        }
     ...
     * @return string
     */
    public function _getJsonPair($key, $value)
    {
